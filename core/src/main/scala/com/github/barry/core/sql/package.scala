...
import com.github.barry.core.sql.sql.JdbcValueAccessor_Option
...
package object sql {
 implicit class SQLStringContext(sc: StringContext) {
  ...
  implicit object JdbcValueAccessor_ArrayBytes extends JdbcValueAccessor[scala.Array[Byte]] {
    override def passIn(stmt: PreparedStatement, index: Int, value: scala.Array[Byte]): Unit = stmt.setBytes(index, value)
    override def passOut(rs: ResultSet, index: Int): scala.Array[Byte] = rs.getBytes(index)
    override def passOut(rs: ResultSet, name: String): scala.Array[Byte] = rs.getBytes(name)
  }

  implicit object JdbcValueAccessor_Array2 extends JdbcValueAccessor[java.sql.Array] {
    override def passIn(stmt: PreparedStatement, index: Int, value: java.sql.Array): Unit = stmt.setArray(index, value)
    override def passOut(rs: ResultSet, index: Int): java.sql.Array = rs.getArray(index)
    override def passOut(rs: ResultSet, name: String): java.sql.Array = rs.getArray(name)
  }
  ...
  /**
    ...
    * JdbcValueAccessor[T]
    *
    * <ul> package com.github.barry.core.sql.sql prdefined a lot of pre-defined implementation for the jdbc value types:
